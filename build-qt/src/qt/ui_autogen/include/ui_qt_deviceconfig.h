...
public:
    ...
    void setupUi(QDialog *DeviceConfig)
    {
        if (DeviceConfig->objectName().isEmpty())
            DeviceConfig->setObjectName("DeviceConfig");
        ...
        QObject::connect(buttonBox, &QDialogButtonBox::accepted, DeviceConfig, qOverload<>(&QDialog::accept));
        QObject::connect(buttonBox, &QDialogButtonBox::rejected, DeviceConfig, qOverload<>(&QDialog::reject));
